(function () {
  ...
  function acceptSeaLandmarkProtected(col, row) {
    ...
    if (window.campaignSystem && typeof window.campaignSystem.requestPartyRestBoon === 'function') {
      window.campaignSystem.requestPartyRestBoon({
        boonKey: 'protected',
        label: 'Protected Landmark Rest',
        sourceKind: 'sea-landmark',
        sourceRef: { col: Number(col), row: Number(row) }
      }).then(function(out) {
  ...
  function acceptSeaDungeonRestBoon(col, row) {
    var hex = seaHexByCoord(col, row);
    if (!hex) return;
    if (window.campaignSystem && typeof window.campaignSystem.requestPartyRestBoon === 'function') {
      window.campaignSystem.requestPartyRestBoon({
    ...
    if (typeof toggleCond === 'function' && S && S.conditions && !S.conditions.empowered) {
      toggleCond('empowered');
    }
    if (typeof advanceDay === 'function') {
      advanceDay(1);
    }
    hex.resultHtml = '<div class="sea-result-title">Sea Ruins - Rest Boon</div><div style="font-size:.82rem;color:var(--muted3);line-height:1.55;">You make camp among the ruins. Long Rest complete, day advanced by 1, and Empowered gained.</div>';
    renderLastSeaInfo(hex);
    showNotif('Sea ruin camp complete. +1 day, Empowered applied.', 'good');
  }
  ...
  window.acceptSeaDungeonRestBoon = acceptSeaDungeonRestBoon;
