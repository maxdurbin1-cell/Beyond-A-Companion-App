...
async function run() {
  ...
  try {
    ...
    if (summary.isCreditsDrop) {
      throw new Error(`Deterministic item-branch test received credits drop unexpectedly: ${JSON.stringify(summary)}`);
    }
    if (!summary.isMerchantItemsDrop) {
      throw new Error(
        "Deterministic item-branch test expected 1-2 merchant item names and received invalid drop. "
...
run().catch((err) => {
  console.error("enemy-loot-drop-items-only smoke failed:", err && err.stack ? err.stack : err);
  process.exitCode = 1;
});
